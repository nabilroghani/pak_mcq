...
export default function MCQs_cart_RightSide({ className = "" }) {
  ...
  return (
    <div className={`${className} w-full`}>
      ...
      {/* Main Container */}
      <div className={`${isMobileMenuOpen ? "block" : "hidden"} md:block sticky top-24 self-start`}>
        <div className="bg-white shadow-xl rounded-2xl p-5 border border-gray-100">
          <div className="flex items-center gap-2 mb-4 border-b pb-2">
            ...
            <h3 className="text-[#1565C0] font-black text-[12px] uppercase tracking-widest">Main Subjects</h3>
          </div>
                ...
                {(openMenu[category.name] || window.innerWidth >= 768) && (
                  <div className="mt-2 flex flex-col gap-1 border-l-2 border-blue-100 ml-4">
                    {category.subCategories.length > 0 ? (
                      ...
                      <p className="text-[10px] text-gray-400 px-4 py-1 italic">No subjects</p>
                    )}
                  </div>
                )}
              </div>
